...
static inline bool PixelExactEqual(const PixelType *p0, const PixelType *p1)
{
    return p0->red == p1->red && p0->green == p1->green && p0->blue == p1->blue && p0->alpha == p1->alpha;
}

template <>
inline bool PixelExactEqual<PF_Pixel8>(const PF_Pixel8 *p0, const PF_Pixel8 *p1)
{
    static_assert(sizeof(PF_Pixel8) == sizeof(uint32_t), "PF_Pixel8 must stay 4 bytes");
    uint32_t a = 0;
    uint32_t b = 0;
    std::memcpy(&a, p0, sizeof(a));
    std::memcpy(&b, p1, sizeof(b));
    return a == b;
}

template <typename PixelType>
static inline unsigned int PixelDelta4(const PixelType *p0, const PixelType *p1)
{
    return static_cast<unsigned int>(ABS(p0->red   - p1->red)) +
           static_cast<unsigned int>(ABS(p0->green - p1->green)) +
           static_cast<unsigned int>(ABS(p0->blue  - p1->blue)) +
           static_cast<unsigned int>(ABS(p0->alpha - p1->alpha));
}
...
{
    return !PixelExactEqual(p0, p1) && PixelDelta4(p0, p1) > range;
}
...
// for Mac
// うまくinline展開してくれないので
#define ComparePixel(p0, p1) (!PixelExactEqual(&(info->in_ptr[p0]), &(info->in_ptr[p1])) && \
                              PixelDelta4(&(info->in_ptr[p0]), &(info->in_ptr[p1])) > info->range)

#define ComparePixelEqual(p0, p1) (PixelExactEqual(&(info->in_ptr[p0]), &(info->in_ptr[p1])) || \
                                   PixelDelta4(&(info->in_ptr[p0]), &(info->in_ptr[p1])) <= info->range)
#endif
